...
class TestProblemViews(TestCase, TestStreamingMixin):
    ...
    def test_admin_changelist_view(self):
        ...
        permission = Permission.objects.get(content_type=content_type, codename="problems_db_admin")
    ...
    def test_admin_add_in_contest_delete_in_problemset(self):
        ...
        url = reverse(
            "oioioiadmin:problems_problem_delete",
            args=(problem.id,),
        )
...
class TestProblemPackageViews(TestCase, TestStreamingMixin):
    ...
    def test_package_traceback_view(self):
        ...
        self.assertTrue(self.client.login(username="test_admin"))
        self.client.get("/c/c/")  # 'c' becomes the current contest
        download_package_traceback_url = reverse("download_package_traceback", kwargs={"package_id": str(package.id)})
...
class TestProblemSite(TestCase, TestStreamingMixin):
    ...
    def test_statement_tab(self):
        url_external_stmt = reverse("problem_site_external_statement", kwargs={"site_key": "123"})
    ...
    def test_statement_replacement(self):
        url = reverse("problem_site", kwargs={"site_key": "123"}) + "?key=replace_problem_statement"
...
def get_submission_left(username, contest_id="c", pi_pk=1):
    ...
    request.user = User.objects.get(username=username) if username is not None else AnonymousUser()
...
class TestProblemSearch(TestCase, AssertContainsOnlyMixin):
    ...
    def test_search_name_multiple(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assert_contains_only(response, ("Zadanko", "Znaczn1k", "Algorytm", "Byteland"))

    def _test_search_name_localized(self, queries, exp_names):
        self.client.get("/c/c/")
        for query, exp_name in zip(queries, exp_names, strict=False):
...
def problem_name(problem, language):
    problem_name = ProblemName.objects.filter(problem=problem, language=language).first()
